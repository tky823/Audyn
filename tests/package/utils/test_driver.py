...
def test_base_trainer_ddp_for_dnr(
    ...
    """Test BaseTrainer for DDP using DnR dataset."""
    BATCH_SIZE = 3
    ITERATIONS = 3
    ...
    with tempfile.TemporaryDirectory(dir=".") as temp_dir:
        ...
        track_names = _save_dummy_dnr(root=feature_dir, num_frames=num_frames)
        ...
        with hydra.initialize(
            ...
            system_name = "cpu_ddp"
            train_name = "dummy_dnr"
            lr_scheduler_name = "dummy"
            optimizer_name = "dummy"

            config = hydra.compose(
                config_name="config",
                overrides=create_dummy_dnr_override(
                    overrides_conf_dir=overrides_conf_dir,
                    exp_dir=exp_dir,
                    list_dir=list_dir,
                    feature_dir=feature_dir,
        ...
        train_dataset_target = "audyn.utils.data.dnr.RandomStemsDNRDataset"
        validation_dataset_target = "audyn.utils.data.dnr.StemsDNRDataset"
        train_dataloader_target = "torch.utils.data.DataLoader"
        validation_dataloader_target = "torch.utils.data.DataLoader"
        ...
        train_dataset_target = "audyn.utils.data.dnr.dataset.DistributedRandomStemsDNRDataset"
        validation_dataloader_target = "audyn.utils.data.dataloader.DistributedDataLoader"
...
def create_dummy_dnr_override(
    ...
    override_list += [
        "train/dataloader=dummy_dnr",
        "train.dataset.train.duration=1",
        "train.dataset.validation.duration=1",
        "model.is_monoral=true",
    ]
...
def _save_dummy_dnr(root: str, num_frames: int) -> Dict[str, List[str]]:
