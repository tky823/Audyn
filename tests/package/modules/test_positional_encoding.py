...
from audyn.modules.positional_encoding import (
...
def test_rotary_positional_embedding(batch_first: bool):
    ...
    batch_size = 2
    length = 8
    base, embed_dim = 20, 10

    positional_encoding = RotaryPositionalEmbedding(base, batch_first=batch_first)
...
@pytest.mark.parametrize("batch_first", parameters_batch_first)
def test_extrapolatable_positional_embedding(batch_first: bool):
    torch.manual_seed(0)
    ...
    positional_encoding = ExtrapolatablePositionalEmbedding(
        invert_decay, smooth, base, batch_first=batch_first
    )
    ...
    # ensure property
    invert_decay = False
    q_positional_encoding = ExtrapolatablePositionalEmbedding(
        invert_decay, smooth, base, batch_first=batch_first
    )
    invert_decay = True
    k_positional_encoding = ExtrapolatablePositionalEmbedding(
        invert_decay, smooth, base, batch_first=batch_first
    )
    ...
    q = q_positional_encoding(q)
    k = k_positional_encoding(k)
...
def test_partial_rotary_positional_embedding(batch_first: bool, fraction: float):
    torch.manual_seed(0)

    batch_size = 2
    length = 8
    base, embed_dim = 20, 16

    partial_positional_encoding = PartialRotaryPositionalEmbedding(
        base, fraction=fraction, batch_first=batch_first
    )
    positional_encoding = RotaryPositionalEmbedding(base, batch_first=batch_first)
    ...
    partial_output = partial_positional_encoding(input)
