...
from audyn.modules.lora import (
...
@pytest.mark.parametrize("batch_first", [True, False])
@pytest.mark.parametrize("persistent", [True, False])
def test_lora_rope_mha(batch_first: bool, persistent: bool) -> None:
    batch_size = 3
    length = 10
    embed_dim, num_heads = 32, 4
    rank = 2

    mha = RotaryPositionalMultiheadAttention(embed_dim, num_heads, batch_first=batch_first)
    lora_mha = LoRARotaryPositionalMultiheadAttention(
        mha.num_heads,
        mha.in_proj_weight,
        in_proj_bias=mha.in_proj_bias,
        bias_k=mha.bias_k,
    ...
    lora_mha = LoRARotaryPositionalMultiheadAttention.build_from_mha(
        mha, rank=rank, persistent=persistent
    )
    lora_output, loar_attn_weights = lora_mha(input, input, input)
...
@pytest.mark.parametrize("batch_first", [True, False])
@pytest.mark.parametrize("persistent", [True, False])
def test_lora_xpos_mha(batch_first: bool, persistent: bool) -> None:
    batch_size = 3
    length = 10
    embed_dim, num_heads = 32, 4
    rank = 2

    mha = ExtrapolatablePositionalMultiheadAttention(embed_dim, num_heads, batch_first=batch_first)
    lora_mha = LoRAExtrapolatablePositionalMultiheadAttention(
        mha.num_heads,
        mha.in_proj_weight,
        in_proj_bias=mha.in_proj_bias,
        bias_k=mha.bias_k,
    ...
    lora_mha = LoRAExtrapolatablePositionalMultiheadAttention.build_from_mha(
        mha, rank=rank, persistent=persistent
    )
    lora_output, loar_attn_weights = lora_mha(input, input, input)
...
@pytest.mark.parametrize("batch_first", [True, False])
@pytest.mark.parametrize("persistent", [True, False])
def test_lora_partial_rope_mha(batch_first: bool, persistent: bool) -> None:
    batch_size = 3
    length = 10
    embed_dim, num_heads = 32, 4
    rank = 2

    mha = PartialRotaryPositionalMultiheadAttention(embed_dim, num_heads, batch_first=batch_first)
    lora_mha = LoRAPartialRotaryPositionalMultiheadAttention(
        mha.num_heads,
        mha.in_proj_weight,
        in_proj_bias=mha.in_proj_bias,
        bias_k=mha.bias_k,
    ...
    lora_mha = LoRAPartialRotaryPositionalMultiheadAttention.build_from_mha(
        mha, rank=rank, persistent=persistent
    )
    lora_output, loar_attn_weights = lora_mha(input, input, input)
