...
class_list_path="../_common/class.txt"
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    while read class_name; do
        ...
        set +e
        tail -n $((${n_validation} + ${n_test})) "${tmp_root}/${class_name}.csv" | head -n ${n_validation} >> "${tmp_root}/validation.csv"
        set -e
        tail -n ${n_test} "${tmp_root}/${class_name}.csv" >> "${tmp_root}/test.csv"
    done < "${class_list_path}"
